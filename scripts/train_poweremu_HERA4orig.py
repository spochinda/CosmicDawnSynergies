...
#with open('output_train.txt', 'a+') as f:
    #f.write("Script started. Rank, size: " + str(rank) + ", " + str(size) + "\n")
...
#T21 = load_files(path, middle="_T21_", name="hera", key='combined_T21s', endings=["mat"])
#xA = load_files(path, middle="_xa_", name="hera", key='combined_xas', endings=["mat"])
#xHI = load_files(path, middle="_xHI_", name="hera", key='combined_xHIs', endings=["mat"])
###SFR = load_files(path, middle="_SFR_", name="hera", key='combined_SFRs', endings=["mat"])
#extra quantities
#Deltak = load_files(path, middle="_Deltak_", name="hera", key='combined_Deltaks', endings=["mat"])
#fII = load_files(path, middle="_fII_", name="hera", key='combined_fIIs', endings=["mat"])
#JA = load_files(path, middle="_JA_", name="hera", key='combined_JAs', endings=["mat"])
#Mcrit = load_files(path, middle="_Mcrit_", name="hera", key='combined_Mcrits', endings=["mat"])
#nu = load_files(path, middle="_nu_", name="hera", key='nu_keV', endings=["mat"])
#SFRII = load_files(path, middle="_SFRII_", name="hera", key='combined_SFRIIs', endings=["mat"])
#SFRIII = load_files(path, middle="_SFRIII_", name="hera", key='combined_SFRIIIs', endings=["mat"])
...
#z = load_files(path, middle="_z_", name="hera", key='z21cm', endings=["mat"])
#without extras
#Deltak_HERA4, [parameters, Trad_HERA4, TK_HERA4, Ts_HERA4] = remove_powerspectra_nans(Deltak, [parameters, Trad, TK, Ts])
...
#print(zmask.shape, kmask.shape)

Deltak_HERA4 = np.delete(Deltak, nan_samples, axis=0)[:,zmask,:][:,:,kmask]
#Deltak_HERA4 = np.delete(arr=Deltak_HERA4, obj=~zmask, axis=1)
#Deltak_HERA4 = np.delete(arr=Deltak_HERA4, obj=~kmask, axis=2)

parameters = np.delete(parameters, nan_samples, axis=0)

Trad_HERA4 = np.delete(Trad, nan_samples, axis=0)[:,zmask]
#Trad_HERA4 = np.delete(arr=Trad_HERA4, obj=~zmask, axis=1)

TK_HERA4 = np.delete(TK, nan_samples, axis=0)[:,zmask]
#TK_HERA4 = np.delete(arr=TK_HERA4, obj=~zmask, axis=1)

Ts_HERA4 = np.delete(Ts, nan_samples, axis=0)[:,zmask]
#Ts_HERA4 = np.delete(arr=Ts_HERA4, obj=~zmask, axis=1)
...
def PT12_to_PL11(PT12):
    # Convert 10 parameters to usually used parameters, with appropriate logs
    features = np.zeros((PT12.shape[0], 11))
...
parameters_HERA4 = PT12_to_PL11(parameters)
...
#offset = 1e-3
    ...
    #T_HERA4 = {"TS":Ts_HERA4, "T21": T21_HERA4, "TK": TK_HERA4, "TR": Trad_HERA4}[key][:,:31]

    #mask = np.all(np.logical_not(np.logical_or(np.isnan(T_HERA4), T_HERA4==0)), axis=-1)
    #print("Using", np.sum(mask), "out of", len(mask), "samples")
    #PL = deepcopy(PL_HERA4[mask])
    #T = T_HERA4[mask]
...
if True: #(rank==0) or (size==1):
    ...
    #i,j,k = np.where(Deltak_HERA4<1)
    #Deltak_HERA4[i,j,k] = 1
    train_y[train_y<1] = 1
...
def calculate_accuracy(emu, test_x, test_y, add_rsd=None):
    ...
    deltas = np.log10((test_y+1e-2)/(pred_y+1e-2))
    return deltas
