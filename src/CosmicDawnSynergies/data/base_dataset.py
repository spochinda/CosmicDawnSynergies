...
class BaseDataset:
    """Dataset for Cosmic Dawn Synergies emulator training.

    Pure numpy/pandas: loads simulation params/targets, interpolates along data
    dimensions, splits train/val, normalizes params, and exposes
    ``params_train/val`` and ``targets_train/val`` as float64 numpy arrays plus
    ``param_stats``. Device placement and batching are the DeviceBatcher's job.

    Seed semantics are unchanged from the torch implementation:
    ``split_seed`` (train/val split), ``gen_data_seed`` (interpolation draws,
    forces n_jobs=1), and optional ``preshuffle_seed`` (one shuffle of the
    generated training set, needed to reproduce legacy runs).
    """
    ...
    def __init__(self, opt: dict):
        ...
        self.params_train, self.params_val, self.targets_train, self.targets_val = train_test_split(
            self.params, self.targets, test_size=self.val_size, train_size=self.train_size,
        ...
        for i, (key, shape) in enumerate(zip(self.data_dims.keys(), dim_shapes)):
            if self.data_dims[key].get("file") is not None:
                self.data_dims[key]["values"] = load_fn(self.data_dims[key]["file"],
                                                        self.data_dims[key].get("key", None)).flatten().astype(np.float64)
            else:
                print(f"File for {key} not provided. Generating {shape} geomspace values from between "
                      f"{self.data_dims[key]['lims_nsample'][0]} and {self.data_dims[key]['lims_nsample'][1]}.")
                self.data_dims[key]["values"] = np.geomspace(*self.data_dims[key]["lims_nsample"][:2],
            ...
            if self.data_dims[key].get("transform") is not None:
                ...
                self.data_dims[key]["values"] = transform_fn(self.data_dims[key]["values"],
                                                             **self.data_dims[key].get("transform_kwargs", {}))
        ...
        self.params_train, self.targets_train = gen_training_data(
            self.params_train, self.targets_train, copy.deepcopy(self.data_dims), n_jobs=n_jobs, verbose=True)
        self.params_val, self.targets_val = prepare_validation_data(
            params=self.params_val, targets=self.targets_val, data_dims=copy.deepcopy(self.data_dims))

        # one-off shuffle of the generated training set (legacy reproduction knob)
        preshuffle_seed = opt.get("preshuffle_seed", None)
        if preshuffle_seed is not None:
            perm = np.random.RandomState(preshuffle_seed).permutation(len(self.params_train))
            self.params_train = self.params_train.iloc[perm].reset_index(drop=True)
            self.targets_train = self.targets_train[perm]
        ...
        # final numpy arrays (float64; cast to float32 at device placement)
        self.params_train = self.params_train.to_numpy()
        self.params_val = self.params_val.to_numpy()
        self.targets_train = np.asarray(self.targets_train)
        self.targets_val = np.asarray(self.targets_val)

    @property
    def in_dim(self):
        return self.params_train.shape[1]
    ...
    def build_param_stats(self):
        ...
        for param, minimum, maximum, mean, std in zip(self.params_train.columns, self.params_mins,
                                                      self.params_maxs, self.params_means, self.params_stds):
