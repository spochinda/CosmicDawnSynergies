...
if True: #(rank==1) or (size==1):
    ...
    tol = 1e-5#1e-5
    offset = 0
    emu_SFR = poweremu(loadfile=None,preprocesss_log_x=False, preprocess_y=True, hidden_layer_sizes=layers, max_iter=9999, solver="adam", offset=0,tol=tol,**{'batch_size':800})
...
if (rank==0) or (size==1):
    ...
    tol = 1e-5#1e-5
    offset = 0
    emu_XRB = poweremu(loadfile=None,preprocesss_log_x=False, preprocess_y=True, hidden_layer_sizes=layers, max_iter=9999, solver="adam", offset=0,tol=tol,**{'batch_size':800})
    print("Checkpoint: XRB training", flush=True)
    emu_XRB.train(train_x, train_y)
    emu_XRB.save(path + "data/trained_emulators_poweremu/CXB1_emu_n{0}_l{1}{2}{3}{4}_t{5}_o{6}.pkl".format(n_over, layers[0], layers[1], layers[2], layers[3], str(tol), str(offset)))
