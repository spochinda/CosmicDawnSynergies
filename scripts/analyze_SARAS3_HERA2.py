...
hera = MAF.load('/data/highz2/HBdata2/harry_stefan_joint_analysis/HERA.pkl')
samples = hera.sample(5000)

assert False

# Load combined data set from Harry
paramNames = ["log10fStar", "log10Vc", "log10fX", "tau", "log10Fr"]
SARASHERA = anesthetic.NestedSamples(root="/data/highz2/HBdata2/harry_stefan_joint_analysis/saras2_saras3_hera_nlive_500/test", columns=paramNames, label="HERA IDR2 + SARAS joint")[paramNames]
assert not np.any(np.isnan(SARASHERA))
assert not np.any(np.isinf(SARASHERA))

SARAS = anesthetic.NestedSamples(root="/data/highz2/HBdata2/harry_stefan_joint_analysis/saras_joint_nlive_500/test", columns=paramNames, label="SARAS joint")[paramNames]
assert not np.any(np.isnan(SARAS))
assert not np.any(np.isinf(SARAS))
...
HERA = anesthetic.samples.MCMCSamples(data=data, columns=paramNames, tex=texDict, limits=priorDict, label='HERA IDR2').iloc[::281]
tmp = HERA.weights
...
fig, ax = HERA.plot_2d(paramNames, alpha=0.5, types={"lower": "kde", "diagonal": "hist", "upper": "scatter"}, diagonal_kwargs={"histtype": "step"})
fig, ax = SARASHERA.plot_2d(ax, alpha=0.5, types={"lower": "kde", "diagonal": "hist", "upper": "scatter"}, diagonal_kwargs={"histtype": "step"})
fig, ax = SARAS.plot_2d(ax, alpha=0.5, types={"lower": "kde", "diagonal": "hist", "upper": "scatter"}, diagonal_kwargs={"histtype": "step"})
plt.legend()
plt.show()
