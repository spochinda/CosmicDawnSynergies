...
TS_emu = poweremu(loadfile="data/trained_emulators_poweremu/Pk_Ts_emu_test.pkl", preprocesss_log_x=False, offset=1e-3)
TK_emu = poweremu(loadfile="data/trained_emulators_poweremu/Pk_TK_emu_test.pkl", preprocesss_log_x=False, offset=1e-3)
TR_emu = poweremu(loadfile="data/trained_emulators_poweremu/Pk_Trad_emu_test.pkl", preprocesss_log_x=False, offset=1e-3)

samples = anesthetic.NestedSamples(root="non-public/idr_chains4/run_IDR4")

if True:
    for key in discrete_params.keys():
        i = samples[key].round().astype("int").tolist()
        samples[key] = np.array(discrete_params[key])[i]

nDerived = 8
nDims = len(paramNames)

fig, ax = samples.plot_2d(paramNames,
                          types={'lower':'hist'},# 'diagonal':'kde'},
                          lower_kwargs={"bins": 20, 'color': ccb[0], "vmin":0, "zorder":-10, "rasterized":True},
                          diagonal_kwargs={'edgecolor': ccb[0]}
                         )
#ax.loc["nu_0","alpha"].set_ylim(0,3000)
#ax.loc["alpha","log10fstarII"].set_ylim(-1.5,-1)
