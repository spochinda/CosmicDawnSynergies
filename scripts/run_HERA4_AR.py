...
priorDict_HERA4 = {
             ...
             "tau": [0.054-3*0.007, 0.054+3*0.007],#Planck+-3sigma
             "log10fradio": np.log10([1e1, 1e5]).tolist(),##############
             "pop": [-0.5, 2.5],#[231, 232, 233],#??
             "feed": [-0.5, 1.5],#[0, 1]
             "delay": [-0.5, 1.5]} #[0, 0.75]

#used to convert from continuos to discrete values
discrete_params = {
            "alpha": [-1, -1.3, -1.5],
...
#Load the trained power spectrum emulator and temperature emulators
...
def loglikelihood(p, include_LWA=False, include_Chandra=False):
    p=np.copy(p)
    for i,(key,val) in enumerate( zip(paramNames,p) ): #convert continous to discrete parameter value
        if key in discrete_params.keys():
            j = round(val)
            p[i] =  discrete_params[key][j]
    m = lambda z,karr,p=p: emulatorModel(z, karr, p)
    logL, individual_loglikes = like_hera.loglike(m, return_individual_loglikes=True)
...
nDerived = bandsNfields+3*len(redshifts) #bandsNfields=logL per field, 3*redshifts=#temperatures saved
settings = PolyChordSettings(nDims, nDerived)
settings.nlive = 10000 #2000
